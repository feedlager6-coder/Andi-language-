...
export default function Dictionary() {
  ...
  const levelColor = (l: string | null | undefined) => {
    if (l === "beginner") return "text-emerald-600 bg-emerald-50";
    if (l === "intermediate") return "text-amber-600 bg-amber-50";
    if (l === "advanced") return "text-rose-600 bg-rose-50";
    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in duration-500">
      ...
      {data && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          {!search && partOfSpeech === "all" && level === "all" && (
              ...
              <Card className="hover-elevate cursor-pointer transition-all hover:border-primary/30 hover:bg-card/70">
                <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div
                      className="p-2 rounded-full text-muted-foreground opacity-30 cursor-not-allowed"
      ...
      <div className="text-xs text-muted-foreground border-t pt-4 opacity-60">
        <span className="inline-flex items-center gap-1">
          ...
          Для подготовки записей — см. PROJECT_PLAYBOOK.md.
        </span>
      </div>
    </div>
  );
